...
import { defineAsyncComponent, reactive, onMounted, computed, ref, nextTick, onBeforeUpdate, onBeforeMount, onUnmounted, watch, toRaw } from 'vue';
...
import { useKeepALiveNames } from '@render/stores/keepAliveNames';
import { useRoutesList } from '@render/stores/routesList';
import { Session } from '@render/utils/storage';
import { isObjectValueEqual } from '@render/utils/arrayOperation';
import other from '@render/utils/other';
import mittBus from '@render/utils/mitt';

// 引入组件
const Contextmenu = defineAsyncComponent(() => import('@render/layout/navBars/tagsView/contextmenu.vue'));
...
const addBrowserSetSession = (
	...
	let temp = [...(tagsViewList ?? [])].map((item) => {
		let child = toRaw(item);
		if (child.component) {
			delete child.component;
		}
		if(!!child.meta) {
			child.meta = toRaw(child.meta);
		}
		if(!!child.query) {
			child.query = toRaw(child.query);
		}
		if(!!child.params) {
			child.params = toRaw(child.params);
		}
		return toRaw(child);
	});
	Session.set('tagsViewList', temp);
};
...
const initTagsView = async () => {
	let isExist = await Session.has('tagsViewList');
	if (isExist && getThemeConfig.value.isCacheTagsView) {
		state.tagsViewList = await Session.get('tagsViewList');
	} else {
		await state.tagsViewRoutesList.map((v: RouteItem) => {
			if (v.meta?.affix && !v.meta.isHide) {
...
const solveAddTagsView = async (path: string, to?: RouteToFrom) => {
	...
	if (current.length <= 0) {
		...
		if (findItem.meta.affix) return false;
...
const singleAddTagsView = (path: string, to?: RouteToFrom) => {
	...
	state.tagsViewList.forEach((v) => {
		if (
			...
			v.url = setTagsViewHighlight(v);
			addBrowserSetSession(toRaw(state.tagsViewList));
		}
	});
};
...
const closeOtherTagsView = async (path: string) => {
	let isExist = await Session.has('tagsViewList');
	if (isExist) {
		state.tagsViewList = [];
		let tags = await Session.get('tagsViewList');
		tags.map((v: RouteItem) => {
			if (v.meta?.affix && !v.meta.isHide) {
				v.url = setTagsViewHighlight(v);
